"""
Database utility module for POLAR-EMS ML Service.
Provides SQLAlchemy connection pooling, Neon URL sanitization, connection health checks,
and station identifier resolution using parameterized SQL.
"""
...
logger = logging.getLogger("polar_ems_ml.db")
...
def get_db_engine() -> Engine:
    ...
    Uses parameterized connection handling with Neon SSL support.
    """
    global _engine
    if _engine is None:
...
def check_db_connection() -> Dict[str, Any]:
    """
    Check database connectivity safely without exposing credentials.
    """
    try:
...
def resolve_station_info(station_identifier: str) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        logger.error(f"Error resolving station '{station_identifier}': {e}")
        return None
